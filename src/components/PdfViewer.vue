...
function computeRotation(rotation: number): number {
  ...
  if (factor > 4) return computeRotation(rotation - 360);
  else if (factor < 0) return computeRotation(rotation + 360);
  return rotation;
}
...
watch(
  () => [props.scale, props.rotation, props.page],
  () => {
    renderPage(props.page);
  }
);
