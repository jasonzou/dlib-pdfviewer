...
async function handleDatabase(messageId, fn, request) {
  if (fn === 'load') {
    ...
    const result = await Database.get(request.path).execute(request.query, request.bindValues)
    return new DToolsResponse(messageId, '', true, new SqlExecuteData(result.rowsAffected, result.lastInsertId))
  } else if (fn === 'select') {
    const result = await Database.get(request.path).select(request.query, request.bindValues)
    return new DToolsResponse(messageId, '', true, result)
  } else {
